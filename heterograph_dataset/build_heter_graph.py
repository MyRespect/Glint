...
alexa2bluep = data_source+"alexa_interact_bluep.csv"
googl2bluep = data_source+"googl_interact_bluep.csv"
...
class HeteroGraphDataset(DGLDataset):
    ...
    def process(self):
        ...
        alx2blu = pd.read_csv(alexa2bluep)
        gog2blu = pd.read_csv(googl2bluep)
        ...
        with open(data_source+'blueprint_embedding.json','r',encoding='utf8') as fp3:
            blueprint_embedding_dict = json.load(fp3)
            for i in range(4001, 4455):
                blueprint_embedding_dict[str(i)] = np.zeros(300)

        with open(data_source+'google_embedding.json','r',encoding='utf8') as fp4:
            google_embedding_dict = json.load(fp4)
            for i in range(4456, 8930):
                google_embedding_dict[str(i)] = np.zeros(512)
        ...
        alx2blu_edge_group = alx2blu.groupby('graph_id')
